...
function KeyRow({ provider, hook, last = false }: { provider: Provider; hook: KeyHook; last?: boolean }) {
  ...
  const customBaseHost = extractHost(customBaseUrl)
  ...
  let collapsedSummary: ReactNode = null
  if (!expanded) {
    if (status === 'valid') {
      collapsedSummary = (
        <div className="flex min-w-0 items-center gap-1.5 text-sm">
          ...
          {customBaseHost ? (
            <>
              ...
              <span className="mono min-w-0 truncate text-(--color-text-3)">{customBaseHost}</span>
            </>
          ) : null}
          {useProxy ? (
            <>
      ...
      collapsedSummary = (
        <div className="flex min-w-0 items-center gap-1.5 text-sm">
          ...
          <span className="min-w-0 truncate text-(--color-danger)">{t('apiKeys.status.invalidShort')}</span>
          <span className="shrink-0 text-(--color-text-4)">·</span>
          <a
            href={providerConfig.apiKeyUrl}
            target="_blank"
      ...
      collapsedSummary = (
        <div className="flex min-w-0 items-center gap-1.5 text-sm">
          ...
          <span className="min-w-0 truncate text-(--color-text-3)">{hint}</span>
          <span className="shrink-0 text-(--color-text-4)">·</span>
          <a
            href={providerConfig.apiKeyUrl}
            target="_blank"
    ...
    <div className={rowClass}>
      <div className="flex min-w-0 items-start gap-3 px-1">
        <span className="flex h-6 shrink-0 items-center">
          <BrandIcon name={providerConfig.brandIcon} size={14} className="relative -top-0.5 text-(--color-text-2)" />
        </span>
        {expanded ? (
          <>
            <div className="min-w-0 flex-1 truncate text-base font-medium text-(--color-text)">{label}</div>
            <a
              href={providerConfig.apiKeyUrl}
              target="_blank"
              rel="noreferrer"
              className="inline-flex h-6 shrink-0 items-center text-sm text-(--color-accent) hover:underline"
            >
              {t('apiKeys.getKey')}
            </a>
          </>
          ...
          <>
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-0.5 min-w-0">{collapsedSummary}</div>
            </div>
            {justValidated ? (
              <span className="inline-flex h-6 shrink-0 items-center gap-1 text-sm font-medium text-(--color-success)">
              ...
              <button
                type="button"
                onClick={handleEdit}
                className="action-soft -mr-1 inline-flex h-6 shrink-0 items-center text-sm"
      ...
      {!expanded && autoProxyEnabled && status === 'valid' && (
        <div
          role="status"
          aria-live="polite"
          className="ml-1 mt-2 flex items-center gap-2 rounded-[var(--radius-sm)] bg-(--color-accent-wash) px-2.5 py-1.5 text-sm text-(--color-accent-text) shadow-[inset_0_0_0_1px_var(--ring-edge-soft)]"
        >
          <Icon name="check" size={12} strokeWidth={2.4} className="shrink-0" />
          <span className="min-w-0 flex-1 leading-snug">{t('apiKeys.proxy.autoEnabledNotice')}</span>
          <button
            type="button"
            onClick={dismissAutoProxyNotice}
            className="icon-btn -mr-1 shrink-0"
            aria-label={t('apiKeys.proxy.autoEnabledDismiss')}
          >
