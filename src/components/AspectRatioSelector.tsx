...
export function AspectRatioSelector({ options, value, resolution, onChange }: Props) {
  ...
  return (
    <div>
      <label className="block text-sm font-medium text-on-surface-variant mb-3">宽高比</label>
      <div className="grid grid-cols-2 gap-1">
        ...
        >
          ...
          <span className={`material-symbols-rounded text-sm transition-transform ${expanded ? 'rotate-180' : ''}`}>expand_more</span>
        </button>
      </div>
    </div>
  )
