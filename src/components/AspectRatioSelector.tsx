    ...
    <div>
      {showLabel ? (
        <div className="flex items-center justify-between mb-1.5">
          ...
          <span className="text-sm text-(--color-text-4)">{pixelLabel(value, resolution)}</span>
        </div>
      ) : (
        ...
        {options.map((option, idx) => {
          ...
          return (
            <div key={option} className="relative group">
              ...
              <div
                className={`pointer-events-none absolute left-1/2 -translate-x-1/2 z-20 px-2 py-1 rounded-[5px] text-base whitespace-nowrap opacity-0 translate-y-0.5 transition-[opacity,translate] duration-150 delay-100 group-hover:opacity-100 group-hover:translate-y-0 ${isBottomRow ? 'bottom-full mb-1.5' : 'top-full mt-1.5'}`}
                style={{
